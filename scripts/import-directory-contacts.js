...
// One-time enrichment: matches rows from a compact open business-directory
// snapshot (name/region/city/address/contacts, no BIN — see
// docs/COMPANIES-EGOV.md for where it comes from) against the official
// companies.sqlite (data.egov.kz, has BIN but no contacts). Matched
// companies get their missing contact fields filled in (never overwrites
// data already present). Unmatched directory rows are inserted as new,
// BIN-less company records — company-quality.js already requires a valid
// 12-digit BIN to be indexable, so these are automatically excluded from
// the public sitemap; they're only reachable through the site's own
// search/catalog.
//
// Reads registry/companies-directory-contacts.json.gz — a flat gzip-JSON
// snapshot (no zip-in-zip, no xlsx parsing needed at runtime; just
// Node's built-in zlib). Regenerate that snapshot with
// scripts/build-directory-snapshot.js if the source archive changes.
//
// Usage:
//   1. Stop Node.js in Plesk.
//   2. node scripts/import-directory-contacts.js --confirm-offline
//   Resumable: progress (next row index) is saved in company_meta after
//   every batch, so a re-run continues instead of restarting.
...
const CONTACT_FIELDS = ['phone', 'mobile_phone', 'email', 'website', 'whatsapp', 'viber', 'telegram', 'work_hours', 'rating', 'review_count', 'lat', 'lon'];

function hasAnyContact(row) {
  return CONTACT_FIELDS.some(key => row[key]);
}
...
function parseArgs(argv) {
  ...
  return {
    ...
    snapshot: value('--snapshot=') || path.join(ROOT, 'registry', 'companies-directory-contacts.json.gz'),
    limitRows: Number.parseInt(value('--limit-rows=') || '', 10) || Infinity,
  };
}
...
async function run() {
  ...
  if (!fs.existsSync(options.snapshot)) throw new Error(`Snapshot not found: ${options.snapshot} (see docs/COMPANIES-EGOV.md)`);

  const db = new DatabaseSync(DB_PATH);
  createSchema(db);

  console.log('[Directory] Loading snapshot...');
  const allRows = loadSnapshot(options.snapshot);
  console.log(`[Directory] ${allRows.length} rows loaded.`);

  const startIndex = Number.parseInt(getMeta(db, 'directory_import_next_row', '0'), 10) || 0;
  const endIndex = Math.min(allRows.length, startIndex + options.limitRows);

  const findCandidates = db.prepare(`
    SELECT c.id, c.name_ru, c.name_kk, c.region_slug
  ...
  // In-run dedup for new inserts: same directory entity often appears under
  // several categories (e.g. a firm listed under both "Юристы" and
  // "Деловые услуги"). Keyed by normalized name + region.
  const seenNew = new Map();

  let matched = 0, inserted = 0, skippedNoName = 0;

  for (let batchStart = startIndex; batchStart < endIndex; batchStart += BATCH_SIZE) {
    const batchEnd = Math.min(endIndex, batchStart + BATCH_SIZE);
    db.exec('BEGIN');
    try {
    ...
    setMeta(db, 'directory_import_next_row', batchEnd);
    console.log(`[Directory] ${batchEnd}/${allRows.length} rows — matched ${matched}, inserted ${inserted}, no-name skipped ${skippedNoName}`);
  }

  if (endIndex >= allRows.length) {
    console.log('[Directory] All rows processed. Rebuilding search index...');
    rebuildSearch(db);
    setMeta(db, 'record_count', db.prepare('SELECT COUNT(*) AS c FROM companies').get().c);
    setMeta(db, 'directory_import_completed_at', nowIso);
  } else {
    console.log(`[Directory] Checkpoint at row ${endIndex}/${allRows.length}. Run again to continue.`);
  }
