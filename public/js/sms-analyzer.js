...
(function (root, factory) {
  ...
  var ROUTES = {
    enis_notary: {
      eyebrow: 'ЕНІС / БНАЖ · уведомление нотариуса',
      title: 'Исполнительная надпись уже совершена — проверьте срок возражения',
      summary: 'Это уведомление нотариальной системы, а не постановление ЧСИ. Само по себе такое SMS не подтверждает, что исполнительное производство уже возбуждено. Для возражения важны копия надписи и подтверждённая дата её получения.',
      steps: [
        'Откройте документ через eGov Mobile либо самостоятельно набранный адрес официального сервиса, скачайте копию надписи и сохраните уведомление.',
        'Проверьте нотариуса, взыскателя, сумму, основание долга и действительно ли требование было бесспорным.',
        'Если не согласны, направьте нотариусу мотивированное письменное возражение с уведомлением в течение десяти рабочих дней со дня получения копии и сохраните подтверждение отправки.',
      ],
      links: [
        { href: '/vozrazhenie-na-ispolnitelnuyu-nadpis', label: 'Как подать возражение', icon: 'bi-file-earmark-text' },
        { href: '/notaries', label: 'Проверить нотариуса', icon: 'bi-book' },
        { href: '/zamena-notariusa', label: 'Если нотариус не работает', icon: 'bi-arrow-left-right' },
      ],
      whatsapp: 'Здравствуйте! Получил(а) уведомление ЕНИС / БНАЖ о совершении исполнительной надписи. Прошу проверить надпись, дату получения и возможные основания для возражения до передачи документа ЧСИ.',
    },
    notary_enforcement: {
  ...
  function normalize(value) {
    return String(value || '')
      ...
      .replace(/[ё]/g, 'е')
      .replace(/[^a-zа-яәғқңөұүһі0-9\s-]/g, ' ')
      .replace(/\s+/g, ' ')
      .trim();
  }
  ...
  function classifySms(value) {
    ...
    var hasEnis = /(?:^|[\s-])(?:бнаж|енис|еніс)(?:[\s-]|$)|enis\s+kz|documentfile\s+ashx/.test(text);
    var hasNotary = /исполнительн[а-я]*\s+надпис|нотариус|атқарушылық\s+жазба|аткарушылык\s+жазба|нотариусқа\s+қарсылық|нотариуска\s+карсылык/.test(text) || hasEnis;
    var hasEnforcement = /исполнительн[а-я]*\s+производств|(?:^|[\s-])чси(?:[\s-]|$)|судебн[а-я]*\s+исполнител|аис\s*оип|aisoip|атқарушылық\s+(?:іс|ис)\s+жүргізу|аткарушылык\s+(?:іс|ис)\s+жургизу|жеке\s+сот\s+орындаушы|(?:^|[\s-])жсо(?:[\s-]|$)/.test(text);
    var hasTravel = /ограничен[а-я]*\s+(?:на\s+)?выезд|запрет[а-я]*\s+(?:на\s+)?выезд|пересечен[а-я]*\s+границ|шетелге\s+шығуға\s+(?:уақытша\s+)?шектеу|қазақстаннан\s+шығуға\s+тыйым/.test(text);
    var hasArrest = /(?:^|\s)арест[а-я]*|заблокирован[а-я]*|инкассов[а-я]*|приостановлен[а-я]*\s+расход|шот[а-яәғқңөұүһі\s]*бұғат|мүлік[а-яәғқңөұүһі\s]*тыйым/.test(text);
    var hasCourt = /исполнительн[а-я]*\s+лист|судебн[а-я]*\s+(?:приказ|решен|акт)|(?:^|\s)суд[а-я]*\s+акт|атқару\s+парағы|сот\s+(?:бұйрығы|шешімі)/.test(text);
    var id = 'unknown';

    if (hasNotary && (hasEnforcement || hasArrest)) id = 'notary_enforcement';
    else if (hasEnis && hasNotary) id = 'enis_notary';
