...
Формирует единый отчёт по открытым сведениям КГД: регистрация, ОКЭД, НДС,
налоговый режим, задолженность, неблагоприятные признаки и статистика.

```json
{ "bin": "260740044168" }
```

Токен КГД используется только сервером и никогда не возвращается клиенту.
...
окружения хостинга (не в коде). Там же задайте `KGD_API_TOKEN` и перезапустите
Node.js-приложение.
